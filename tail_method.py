...
class methodGEV():
    ...
    def fitGEVmoments(self,logTmaxima,xiRange):
    ...
    def  bootstrapGEVmoments(self,logTmaxima,xiRange,error=0.01):
        XI = []
        M = []
        S = []
        MAX = []
    ...
    def xi_vs_nRange(self,df,Trange,xiRange):    
    ...
    def estimateGPDparameters(self,sigma,mu,s,T,zeta,Lambda):
        '''Formula (11) and (12)'''
    ...
    def psi(self,Lambda,time_interval,xi,H,s,x):
...
class methodGBD():
    ...
    def makeBootstrapHsample(self,dicHsamples):
    ...
    def bootstratpGPDsample(self,dicHsamples,n):
        '''bootstrap by replacement : 
        pick up randomly r times one value from the set M1,...,Mr'''
        
        for i in range(n):
            '''make a sample'''
            sample = self.makeBootstrapHsample(dicHsamples)
            '''make GPD estimation of this sample'''
            parameters = fitGPD(self,sample)
